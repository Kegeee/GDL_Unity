...
public class ValidateRotationButton : MonoBehaviour
{
    ...
    private void OnValidateButton()
    {
        try
        {
            ...
            selectedTrial.RotationOffset = new Vector3(x, y, z);

            Debug.Log($"Rotation offset of trial {selectedTrial.TrialId} has been " +
                $"saved to : {selectedTrial.RotationOffset}.");
        }
        catch
        {
            Debug.LogWarning("Could not convert value to Rotation Offset.");
        }
    }
}
